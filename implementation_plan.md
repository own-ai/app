...
6. **Create sandboxed Rhai engine** - Create `tools/rhai_engine.rs` with `create_sandboxed_engine()`. Set max_operations (100,000), max_string_size (1MB), max_array_size (10,000). Register safe Rust functions: `http_get`, `http_post`, `safe_read_file`, `safe_write_file`. All HTTP functions require HTTPS. All file functions are workspace-scoped.

7. **Create Tool Registry with DB storage** - Create `tools/registry.rs` with `RhaiToolRegistry`. Add `tools` and `tool_executions` tables to `database/schema.rs`. Implement register, load (with compile cache), execute (with execution logging), list, delete, reload_all.

8. **Create RhaiExecuteTool** - Create `tools/rhai_bridge_tool.rs` implementing `rig::tool::Tool`. This tool accepts a tool name and parameters, looks up the Rhai script in the registry, and executes it. This is how the LLM agent invokes dynamic tools.

9. **Create Code Generation Agent** - Create `tools/code_generation.rs` with `CodeGenerationAgent`. It uses the same LLM provider to generate Rhai scripts from natural language requirements. Includes `validate_script()` (compile + safety check) and `extract_metadata()` (name, description, parameters from the script).

10. **Integrate Rhai tools with agent** - Modify `create_tools()` in `agent/mod.rs` to include RhaiExecuteTool. Update `OwnAIAgent::system_prompt()` to dynamically list available Rhai tools. Add Tauri commands in `commands/tools.rs` for tool management.

11. **Add Capability Detection** - Enhance the system prompt to instruct the agent when and how to create new tools. The agent should recognize when it lacks a capability and use the code generation flow to create a Rhai script. This is primarily a system prompt enhancement, not new code.

### Phase 4: Deep Agent Features (7 steps)

12. **Canvas System - Backend** - Create `canvas/mod.rs` with program CRUD operations. Programs are stored as HTML files at `~/.ownai/instances/{id}/programs/{program-name}/index.html` with a `metadata.json`. Add `programs` and `program_data` tables to DB schema. Create Tauri commands in `commands/canvas.rs`.

13. **Canvas System - Custom Protocol** - Configure Tauri to serve program files via a custom `ownai://` protocol (or use `asset:` protocol). This allows the iframe to load HTML apps from the instance's programs directory. Update `tauri.conf.json` and `capabilities/default.json`.

14. **Canvas System - Frontend** - Create `Canvas.tsx` (sandboxed iframe), `CanvasView.tsx` (split-view layout), `ProgramList.tsx`. Add `canvasStore.ts`. Modify `App.tsx` to support Canvas view toggle. Add i18n translations.

15. **Bridge API** - Create `canvas/bridge.rs` to handle postMessage communication between Canvas iframes and the Rust backend. Implement: chat(), storeData(), loadData(), notify(), readFile(), writeFile(). Inject `window.ownai` JavaScript bridge into served HTML programs.
