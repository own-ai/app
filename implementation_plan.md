...
16. **Sub-Agent System** - ✅ COMPLETE - Created `tools/subagents.rs` with dynamic sub-agent system and `tools/memory_tools.rs` with 3 new rig Tools. Architecture evolved from predefined sub-agents to fully dynamic: the main agent creates sub-agents on the fly via `DelegateTaskTool` (name="delegate_task") with custom system prompts and task descriptions. Sub-agents get ALL tools except `delegate_task` itself (to prevent recursion). Created `ClientProvider` enum wrapping Anthropic/OpenAI/Ollama clients. Created `base_tools_prompt()` for tool documentation -- both the main agent's `system_prompt()` and sub-agent prompts call this function. Refactored `LongTermMemory` to `SharedLongTermMemory = Arc<Mutex<LongTermMemory>>` for concurrent access by tools. Created 3 memory rig Tools: `SearchMemoryTool` (semantic search), `AddMemoryTool` (store entries), `DeleteMemoryTool` (remove entries) -- available to ALL agents. Updated `ContextBuilder` to use `SharedLongTermMemory`. Updated all 3 provider match arms (Anthropic/OpenAI/Ollama) in `OwnAIAgent::new()` with new `create_tools()` signature (10 args including SharedLongTermMemory, ClientProvider, model). Updated `extract_and_store_facts()` to use shared lock pattern. Updated `commands/memory.rs` to clone SharedLongTermMemory and lock (releases AgentCache before awaiting). Refactored main agent `system_prompt()` to use `base_tools_prompt()` + identity section + delegation section. 21 tools total for main agent, 20 for sub-agents. 14 new tests (9 memory tools + 5 subagents). All 154 tests pass, cargo clippy clean, cargo fmt clean.

17. **Scheduled Tasks** - ✅ COMPLETE - Created `scheduler/` module with 4-file structure: `mod.rs` (Scheduler, SharedScheduler, ScheduledTask, validate_cron_expression, 5 tests), `storage.rs` (DB CRUD: load_tasks, save_task, delete_task, update_task_last_run, set_task_enabled, get_task, 7 tests), `runner.rs` (register_task_job, execute_task, run_task_agent with provider-specific agents, load_and_register_instance_tasks), `tools.rs` (CreateScheduledTaskTool, ListScheduledTasksTool, DeleteScheduledTaskTool, 9 tests). Added `scheduled_tasks` table to DB schema. Added `tokio-cron-scheduler` v0.15.1 and `croner` v3.0.1 dependencies. Scheduler initialized as SharedScheduler in `lib.rs` setup, loads tasks for all instances on startup. 3 scheduler Tauri commands (list, delete, toggle). 3 scheduler rig Tools added to agent via `app_handle.try_state()`. Scheduled task agents use `build_sub_agent_tools()` (same tools as sub-agents). Frontend: ScheduledTask TypeScript type, i18n translations (EN + DE). System prompt updated with Scheduled Tasks documentation via `base_tools_prompt()`. 31 Tauri commands total, 24 tools for main agent. All 175 tests pass, cargo clippy clean.

18. **Dynamic System Prompt** - ✅ COMPLETE - The system prompt is now fully dynamic. `OwnAIAgent::system_prompt()` calls `base_tools_prompt()` which documents all tools: filesystem, planning, dynamic Rhai tools, self-programming, Canvas programs with Bridge API, memory tools, sub-agent delegation, and scheduled tasks. Available Rhai tools are dynamically listed from the registry. The prompt is assembled based on what's available in the current agent instance.
