...
11. **Add Capability Detection** - Enhance the system prompt to instruct the agent when and how to create new tools. Update `OwnAIAgent::system_prompt()` to dynamically list available Rhai tools. The agent should recognize when it lacks a capability and use the code generation flow to create a Rhai script. This is primarily a system prompt enhancement, not new code.
