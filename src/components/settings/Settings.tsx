...
export const Settings = ({ isOpen, onClose }: SettingsProps) => {
  ...
  const { providers, loadProviders, saveApiKey, deleteApiKey } = useInstanceStore();
...
const APIKeyRow = ({ providerName, hasKey, onSave, onDelete }: APIKeyRowProps) => {
  const { t } = useTranslation();
        ...
        <div className="space-y-3">
          <div className="relative">
            <Input
              ...
              placeholder={t('settings.api_key_placeholder', { provider: providerName })}
              disabled={isSaving}
              autoFocus
            />
            <button
