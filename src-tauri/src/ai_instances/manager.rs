...
impl AIInstanceManager {
    ...
    fn load_instances() -> Result<HashMap<String, AIInstance>> {
        ...
        let contents = fs::read_to_string(&config_path)
            .context("Failed to read instances config")?;
        
        let instances: HashMap<String, AIInstance> = serde_json::from_str(&contents)
            .context("Failed to parse instances config")?;
    ...
    fn save_instances(&self) -> Result<()> {
        ...
        fs::write(&config_path, contents)
            .context("Failed to write instances config")?;
